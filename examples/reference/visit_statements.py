...
@_Jac.make_architype(
    "walker", on_entry=[_Jac.DSFunc("create", _Jac.RootType)], on_exit=[]
)
class creator:
...
@_Jac.make_architype(
    "node", on_entry=[_Jac.DSFunc("func_1", creator | Travellor)], on_exit=[]
)
class item:
    val: int
    ...
    def func_1(self, _jac_here_: creator | Travellor) -> None:
        ...
        if _Jac.visit_node(
            _jac_here_, _Jac.edge_ref(self, _Jac.EdgeDir.OUT, None, None)
        ):
            pass
        else:
