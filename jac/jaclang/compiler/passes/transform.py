...
class Transform(ABC, Generic[T, R]):
    ...
    def __init__(self, ir_in: T, prog: JacProgram) -> None:
        ...
        self.cur_node: UniAstNode = ir_in  # tracks current node during traversal
        self.prog = prog
        self.time_taken = 0.0
        self.ir_in: T = ir_in
    ...
    def log_error(self, msg: str, node_override: Optional[UniAstNode] = None) -> None:
        """Pass Error."""
        alrt = Alert(
            msg,
    ...
    def log_warning(self, msg: str, node_override: Optional[UniAstNode] = None) -> None:
        """Pass Error."""
        alrt = Alert(
            msg,
            self.cur_node.loc if not node_override else node_override.loc,
