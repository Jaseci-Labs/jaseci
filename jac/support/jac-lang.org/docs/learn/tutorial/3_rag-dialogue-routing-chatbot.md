...
```jac# RAG + Dialogue Routing Chatbot (Part 3/3)
Now that you have built a RAG chatbot, you can enhance it by adding dialogue routing capabilities. Dialogue routing is the process of directing the conversation to the appropriate dialogue model based on the user's input.
...
First, define an enum ChatType to represent the possible types of dialogue states your chatbot can handle. This classification is crucial for routing queries effectively:
...
```
The classify ability uses an LLM to analyze the query and assign it a ChatType.
Key Parameter: The temperature=0.0 ensures deterministic behavior during classification. 
...
It decides whether the query needs document retrieval (RAG), simple question answering (QA), or another task (e.g., SUMMARY).
Think of the Router as a traffic controller, deciding which "lane" (or model) the query should take.
How does it work?
The classify ability calls an LLM, passing the query (message) to analyze its content and intent.
The llm(method="Reason", temperature=0.0) uses reasoning to determine the classification. A low temperature value makes the decision consistent and predictable.
