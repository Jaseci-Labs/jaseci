...
def config_setup():
    ...
    if model is None:
        ...
        model = models.PolyEncoderModelShared(
            config=bert_config, model_name=model_name, poly_m=poly_m, shared=shared
        )
    elif save_restart:
...
@jaseci_action(act_group=["poly_enc"], aliases=["get_poly_cos_sim"], allow_remote=True)
def cosSimilarityScore(context_embedding, candidate_embedding):
    ...
    final_context_vec = models.dot_attention(
        candidates_vec, context_vecs, context_vecs, None, None
    )
...
@jaseci_action(act_group=["poly_enc"], aliases=["inference"], allow_remote=True)
def getinference(contexts, candidates):
    global model
    model.eval()
    predicted_label = evaluate.get_inference(
        model, tokenizer, context=contexts, candidate=candidates
    )
...
@jaseci_action(
    act_group=["poly_enc"], aliases=["getcontextembedding"], allow_remote=True
)
...
@jaseci_action(
    act_group=["poly_enc"], aliases=["getcandidateembedding"], allow_remote=True
)
...
@jaseci_action(act_group=["poly_enc"], aliases=["setconfig"], allow_remote=True)
