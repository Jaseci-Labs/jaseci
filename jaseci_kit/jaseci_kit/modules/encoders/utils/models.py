...
class PolyEncoderModelShared(nn.Module):
    ...
    def forward(
        ...
        if get_embedding and context_data is not None:
            context_input_ids, context_segment_ids, context_input_masks = (
                context_data["context_input_ids"],
            ...
            state_vecs = self.cont_model(
                context_input_ids.unsqueeze(0),
                context_input_masks.unsqueeze(0),
                context_segment_ids.unsqueeze(0),
            )[0]
            ...
            candidates_input_ids, candidates_segment_ids, candidates_input_masks = (
                candidate_data["candidate_input_ids"],
            ...
            state_vecs = self.cand_model(
                candidates_input_ids, candidates_input_masks, candidates_segment_ids
            )[0]
            ...
            candidates_vec = dot_attention(
                poly_codes, state_vecs, state_vecs, candidates_input_ids, self.dropout
            )
        ...
        poly_codes = self.poly_code_embeddings(poly_code_ids)
        candidates_vec = dot_attention(
            poly_codes, state_vecs, state_vecs, candidates_input_masks, self.dropout
        )
        candidates_vec = candidates_vec.view(batch_size, res_cnt, -1)
        ...
        if labels is not None:
            candidates_vec = candidates_vec.view(1, batch_size, -1).expand(
