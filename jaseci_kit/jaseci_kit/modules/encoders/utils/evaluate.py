...
def get_inference(model, tokenizer, contexts, candidates, train_config):
    ...
    candidate_transform = token_util.SelectionSequentialTransform(
        tokenizer=tokenizer,
        max_len=train_config['max_candidate_length']
    )
...
def get_embeddings(model, tokenizer, text_data,
                   train_config, embed_type="context"):
    if embed_type == "context":
        ...
        context_data = token_util.EvalDataset(
            text_data,
            context_transform=context_transform,
            candidate_transform=None, mode=embed_type)
    else:
        candidate_transform = token_util.SelectionSequentialTransform(
            tokenizer=tokenizer,
            max_len=train_config['max_candidate_length'])
    ...
    for step, batch in enumerate(dataloader, start=1):
        ...
        with torch.no_grad():
            if embed_type == "context":
                embeddings = model(context_input_ids=token_ids_batch,
                                   context_input_masks=input_masks_batch,
                                   get_embedding=embed_type,
                                   mode="get_embed")
            else:
                embeddings = model(candidate_input_ids=token_ids_batch,
