...
class PyastBuildPass(Pass[ast.PythonModuleAst]):
    ...
    def proc_ann_assign(self, node: py_ast.AnnAssign) -> ast.Assignment:
        ...
        print(
            ...
            isinstance(annotation, ast.Expr),
        )
        if (isinstance(value, (ast.Expr, ast.YieldExpr)) or not value) and isinstance(
            annotation, ast.Expr
        ):
            return ast.Assignment(
                target=target,
    ...
    def proc_bool_op(self, node: py_ast.BoolOp) -> ast.BoolExpr:
        ...
        valid_values = ast.SubNodeList[ast.Expr](items=valid, delim=",", kid=values)
    ...
    def proc_import_from(self, node: py_ast.ImportFrom) -> ast.Import:
        ...
        items = (
            ast.SubNodeList[ast.ModuleItem](
                items=valid_names, delim=",", kid=valid_names
            )  # delim needed
            if valid_names
            else None
        )
        ...
        return ast.ListVal(
            values=(
                ast.SubNodeList[ast.Expr](
                    items=valid_elts, delim=",", kid=valid_elts
                )  # delim needed
                if valid_elts
                else None
            ),
    ...
    def proc_pass(self, node: py_ast.Pass) -> ast.SubNodeList:
        ...
        braces = ast.SubNodeList[ast.CodeBlockStmt | ast.ArchBlockStmt](  # delim needed
            items=[], delim=",", kid=[l_brace, r_brace]
        )
    ...
    def proc_set(self, node: py_ast.Set) -> ast.SetVal:
        ...
        if len(node.elts) != 0:
            ...
            valid_elts = ast.SubNodeList[ast.Expr](items=valid, delim=",", kid=valid)
            kid = valid
        else:
            valid_elts = None
            l_brace = self.operator(Tok.LBRACE, "{")
    ...
    def proc_arguments(self, node: py_ast.arguments) -> ast.FuncSignature:
        ...
        fs_params = ast.SubNodeList[ast.ParamVar](
            items=valid_params, delim=",", kid=valid_params
        )  # delim needed
