...
class TestJacLangServer(TestCase):
    ...
    def test_completion(self) -> None:
        ...
        workspace = Workspace(workspace_path, lsp)
        lsp.lsp._workspace = workspace
        game_main = uris.from_fs_path(
            self.fixture_abs_path("base_module_structure.jac")
        )
        lsp.quick_check(game_main)
        lsp.deep_check(game_main)
        lsp.type_check(game_main)
        completions_1 = lsp.get_completion(game_main, lspt.Position(37, 16), completion_trigger='.').items
        out_1 = (['get_color','color','point'],3)
        for i in out_1[0]:
            self.assertIn(i, str(completions_1))
            self.assertEqual(out_1[1], len(completions_1))
        completions_2 = lsp.get_completion(game_main, lspt.Position(39, 16), completion_trigger='a').items
        self.assertIn('add', str(completions_2))
        self.assertEqual(1, len(completions_2))
        completions_3 = lsp.get_completion(game_main, lspt.Position(37, 16), completion_trigger='.').items
        out_3 = (['get_color','color','point'],3)
        for i in out_1[0]:
            self.assertIn(i, str(completions_1))
            self.assertEqual(out_1[1], len(completions_1))
