...
def helper(node:NodeArchitype,cur_depth:float,connections:list,visited_nodes:list,queue:list,bfs:bool,dfs:Callable,depth:float,node_limit:int,edge_limit:int):
        ...
        for edge in node._jac_.edges:
            ...
            if is_self_loop: continue #lets skip self loop for a while      
                ...
                new_con=(node._jac_, other_nd._jac_,edge) if not is_in_edge else (other_nd._jac_, node._jac_,edge)
