...
class OrmPrivateTests(TestCaseHelper, TestCase):
    ...
    def test_jsci_db_to_engine_hook_on_authenticate_saving(self):
        ...
        tnode = node.Node(m_id="anon", h=user._h, sent=None)
        tnode.name = "GOBBY"
    ...
    def test_jsci_db_to_engine_hook_on_authenticate_loading(self):
        ...
        user = self.user
        self.assertIsNotNone(user._h)
        temp_id = node.Node(m_id=user.master.urn, h=user._h, sent=None).id
        h = user._h
        h.commit()
    ...
    def test_jsci_db_to_engine_hook_on_authenticated_transacting(self):
        ...
        user = self.user
        self.assertIsNotNone(user._h)
        temp_id = node.Node(m_id="anon", h=user._h, sent=None).id
        ...
        oedge = otnode.attach_outbound(
            node.Node(m_id=otnode._m_id, h=otnode._h, sent=None)
        )
    ...
    def test_parent_ids_stored_and_loaded_as_uuid(self):
        ...
        """
        user = self.user
        node1 = node.Node(m_id="anon", h=user._h, sent=None)
        node2 = node.Node(m_id="anon", h=user._h, parent_id=node1.id, sent=None)
        user._h.commit()
    ...
    def test_redis_saving(self):
        ...
        tnode = node.Node(m_id="anon", h=self.user._h, sent=None)
        tnode.name = "GOBBY"
        tnode.save()  # Jaseci object save
        tnode._h.commit()
    ...
    def test_redis_loading(self):
        ...
        """
        nd = node.Node(m_id="anon", h=self.user._h, sent=None)
        temp_id = nd.id
        nd._h.commit()
    ...
    def test_redis_transacting(self):
        ...
        otnode.name = "Rizzoou"
        oedge = otnode.attach_outbound(
            node.Node(m_id="anon", h=self.user._h, sent=None)
        )
        otnode.save()  # Jaseci object save
        otnode._h.commit()
