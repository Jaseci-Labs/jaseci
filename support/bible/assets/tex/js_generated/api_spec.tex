...
primary API used to run walkers is \textbf{walker\_run}.
...
\apispec{cli: walker run | api: walker\_run | auth: user}{name: str (*req), nd: node (None), ctx: dict (\{\}), _req_ctx: dict (\{\}), snt: sentinel (None), profiling: bool (False)}
