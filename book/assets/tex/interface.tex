...
\par
First, we want to create our default graph. Remember, all walkers, our primary unit of computation, must run on a node. By default we can use the root node of a given graph, so we need to create a base graph. Now, let's say we're a bit rusty and have forgotten how create our initial graph. We can first navigate the help many to figure it out.
\par
