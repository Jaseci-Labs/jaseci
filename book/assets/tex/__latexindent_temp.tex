...
\par
Ok, lets start with a scenario. Say you'd like to write your first Jac program which will include some nodes, edges, and walkers and you'd like to see some standard output and what the graph looks like after you run an interesting walker. Let role play.
\par
Lets hop into a \texttt{jsctl} shell.
\par
\shellout{wf_init.shell}
\par
Good, we're in! And we've set the session to be an in-memory session so no session file will be saved. At the moment we only care about the Jac program we write, which will be saved, but the state of the Jaseci machine we'll run it on doesn't really matter to us.
\par
Now that we've got our shell running, we first want to create a blank graph. Remember, all walkers, Jaseci's primary unit of computation, must run on a node. By default we can use the root node of a freshly created graph, so we need to create a base graph. But oh no! We're a bit rusty and have forgotten how create our initial graph. Let's navigate the help menu to jog our memories.
\par
\shellout{wf_help_nav.shell}
\par
Ohhh yeah! That's it. After simply using \texttt{help} from the shell we were able to navigate to the info for \texttt{graph create}. Let's use our newly gotten wisdom.
\par
...
\par 
Next, lets write some Jac code for our little program.
\par
