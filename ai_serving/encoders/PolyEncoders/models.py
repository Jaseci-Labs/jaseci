...
class PolyEncoderModelShared(nn.Module):
  ...
  def forward(self, context_data=None,candidate_data=None, labels=None,eval=False,get_embedding=False):
    if get_embedding and context_data is not None:
        ...
        state_vecs = self.cont_cand_model(context_input_ids.unsqueeze(0), context_input_masks.unsqueeze(0), context_segment_ids.unsqueeze(0))[0]  # [bs, length, dim]
        ...
        state_vecs = self.cont_cand_model(candidates_input_ids, candidates_input_masks, candidates_segment_ids)[0]  # [bs,dim]
    ...
    state_vecs = self.cont_cand_model(context_input_ids, context_input_masks, context_segment_ids)[0] 
    ...
    state_vecs = self.cont_cand_model(candidates_input_ids, candidates_input_masks, candidates_segment_ids)[0]  # [bs, length, dim]
