...
import os
import configparser
from models import PolyEncoderModelShared,dot_attention
...
def config_setup():
    ...
    elif (not shared) and (model is None):
        model = PolyEncoderModelShared(config=bert_config, model_name=model_name, poly_m=poly_m)
    model.to(device)
config_setup()
...
def cosSimilarityScore(request_data : ScoreEmbeddings):
    ...
    final_context_vec = dot_attention(candidates_vec, context_vecs, context_vecs, None, None)
