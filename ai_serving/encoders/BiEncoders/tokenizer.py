...
class SelectionDataset(Dataset):
  def __init__(self,contexts=None,candidates=None, context_transform=None, candidate_transform=None, sample_cnt=None):
    ...
    print(candidates, contexts)
    if (contexts is not None) and (candidates is not None):
            ...
            if len(candidates)>1:
                ...
                for value in candidates:
                    if value not in candidate: 
                        ...
                        group['labels'].append(lbl)
                        group['context'] = context
    elif candidates is not None and contexts is None:
        for candidate in candidates:
            ...
            group['context'] = None
            self.data_source.append(group)
    elif contexts is not None and candidates is None:
