...
class ModelManagerTestCase(unittest.TestCase):
    ...
    def test_create_version_path_creates_directory(self):
        save_path = self.manager.create_version_path()
        self.assertTrue(save_path.exists())

    def test_create_version_path_returns_different_paths(self):
        save_path_1 = self.manager.create_version_path()
        save_path_2 = self.manager.create_version_path()
        self.assertNotEqual(save_path_1, save_path_2)

    def test_get_version_path_returns_correct_path(self):
        save_path = self.manager.create_version_path()
        load_path = self.manager.get_version_path()
    ...
    def test_get_version_ids_returns_version_names_in_order(self):
        self.manager.create_version_path()
        self.manager.create_version_path()
