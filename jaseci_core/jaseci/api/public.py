...
def public_interface_to_api(hook, params, api_name):
    """
    ...
    """
    param_map = {}
    if (api_name not in globals().keys()):
        logger.error(f'{api_name} not a valid API')
        return False
    func_sig = signature(globals()[api_name])
    for i in func_sig.parameters.keys():
        ...
        if (p_name in params.keys()):
            ...
            if (issubclass(p_type, element)):
                ...
                val = hook.get_obj('override', uuid.UUID(val), override=True)
    ...
    ret = globals()[api_name](**param_map)
...
def get_public_api_signature(api_name):
    """
    ...
    if (api_name not in globals().keys()):
        ...
        return signature(globals()[api_name])


def public_api_walker_summon(key: str, walk: walker, nd: node,
                             ctx: dict = {}):
    """
