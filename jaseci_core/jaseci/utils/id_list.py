...
class id_list(list):
    ...
    def get_obj_by_name(self, name, kind=None, silent=False):
        ...
        for i in self:
            if (not self.parent_obj._h.has_obj(uuid.UUID(i))):
                ...
                healing.append(i)
                continue
            obj = self.parent_obj._h.get_obj(uuid.UUID(i))
            if(obj.name == name):
                if(kind and obj.kind != kind):
                    continue
                ret = self.parent_obj._h.get_obj(uuid.UUID(i))
                break
        self.heal(healing)
    ...
    def obj_list(self):
        ...
        for i in self:
            ...
            if (not obj):
                ...
                ret.append(self.parent_obj._h.get_obj(uuid.UUID(i)))
        self.heal(healing)
        return ret
    ...
    def first_obj(self):
        ...
        if(not self):
            ...
            return None
        return self.parent_obj._h.get_obj(uuid.UUID(self[0]))
