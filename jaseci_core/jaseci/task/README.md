...
Any walker that can be called with `async` field

### **GET**
 - just add `async=true` on query param

### **POST** (just choose one)
 - add `async=true` query param
 - if json body, add `"async":true`
 - if multipart, add new field `async` value is equal to `true`
