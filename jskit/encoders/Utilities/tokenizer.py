...
class SelectionSequentialTransform(object):
    def __init__(self, tokenizer, max_len=128, max_history=10,
                 pair_last=False):
    ...
    def __call__(self, texts):
        ...
        for text in texts:
            ...
            input_ids, segment_ids, input_masks = tokenized_dict[
                'input_ids'], tokenized_dict[
                    'token_type_ids'], tokenized_dict['attention_mask']
        ...
        return input_ids_list, segment_ids_list, \
            input_masks_list, contexts_masks_list

    def __str__(self) -> str:
        return 'maxlen%d_maxhistory%d_pairlast%s' % (
            self.max_len, self.max_history, str(self.pair_last))
...
class SelectionJoinTransform(object):
    ...
    def __call__(self, texts):
        ...
        for text in texts[::-1][:self.max_history]:
            ...
            input_ids, input_masks = tokenized_dict[
                'input_ids'], tokenized_dict['attention_mask']
...
class SelectionDataset(Dataset):
    def __init__(self, contexts=None, candidates=None, context_transform=None,
                 candidate_transform=None, sample_cnt=None):
    ...
    def __get_single_item__(self, index):
        if index in self.transformed_data:
            ...
            context, candidates, labels = group['context'], group[
                'candidates'], group['labels']
    ...
    def batchify_join_str(self, batch):
        contexts_token_ids_list_batch, contexts_segment_ids_list_batch, \
            contexts_input_masks_list_batch, candidates_token_ids_list_batch, \
            candidates_segment_ids_list_batch, \
            candidates_input_masks_list_batch = [], [], [], [], [], []
        labels_batch = []
        for sample in batch:
            (contexts_token_ids_list, contexts_segment_ids_list,
                contexts_input_masks_list), (candidates_token_ids_list,
                                             candidates_segment_ids_list,
            ...
            candidates_segment_ids_list_batch.append(
                candidates_segment_ids_list)
        ...
        return contexts_token_ids_list_batch, \
            contexts_segment_ids_list_batch, contexts_input_masks_list_batch, \
            candidates_token_ids_list_batch, \
            candidates_segment_ids_list_batch, \
...
class ContextData():
    def __init__(self, text, context_transform):
        ...
        transformed_context = self.context_transform(self.context_list)
    ...
    def get_data(self):
        # print(self.transformed_data)
        (contexts_token_ids_list, contexts_segment_ids_list,
         contexts_input_masks_list) = self.transformed_data
        long_tensors = [contexts_token_ids_list,
                        contexts_segment_ids_list, contexts_input_masks_list]
        contexts_token_ids_list, contexts_segment_ids_list,\
            contexts_input_masks_list = (
                torch.tensor(t, dtype=torch.long) for t in long_tensors)
        return contexts_token_ids_list, contexts_segment_ids_list, \
            contexts_input_masks_list
...
class CandidateData():
    def __init__(self, candidates, candidate_transform):
        ...
        transformed_candidate = self.candidate_transform(self.candidate_list)
    ...
    def get_data(self):
        ...
        (candidate_token_ids_list, candidate_segment_ids_list,
         candidate_input_masks_list, _) = self.transformed_data
        long_tensors = [candidate_token_ids_list,
                        candidate_segment_ids_list, candidate_input_masks_list]
        [candidate_token_ids_list, candidate_segment_ids_list,
            candidate_input_masks_list] = (
            torch.tensor(t, dtype=torch.long) for t in long_tensors)
        return [candidate_token_ids_list,
