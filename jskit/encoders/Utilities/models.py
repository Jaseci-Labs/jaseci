...
class PolyEncoderModelShared(nn.Module):
  ...
  def forward(self, context_data=None,candidate_data=None, labels=None,eval=False,get_embedding=False):
    if get_embedding and context_data is not None:
        ...
        print( res_cnt, seq_length)
        state_vecs = self.cont_model(context_input_ids.unsqueeze(0), context_input_masks.unsqueeze(0), context_segment_ids.unsqueeze(0))[0]  # [bs, length, dim]
        ...
        context_vec = dot_attention(poly_codes, state_vecs, state_vecs, context_input_masks.unsqueeze(0), self.dropout)
        ...
        state_vecs = self.cand_model(candidates_input_ids, candidates_input_masks, candidates_segment_ids)[0]  # [bs,dim]
        poly_code_ids = torch.zeros(batch_size * res_cnt, 1, dtype=torch.long, device=candidates_input_ids.device)
        poly_codes = self.poly_code_embeddings(poly_code_ids)
        candidates_vec = dot_attention(poly_codes, state_vecs, state_vecs, candidates_input_ids, self.dropout)
    ...
    state_vecs = self.cont_model(context_input_ids, context_input_masks, context_segment_ids)[0] 
    batch_size, res_cnt, seq_length = candidates_input_ids.shape
    # state_vecs = self.cont_cand_model(context_input_ids, context_input_masks, context_segment_ids)[0]  # [bs, length, dim]
    poly_code_ids = torch.arange(self.poly_m, dtype=torch.long, device=context_input_ids.device)
    poly_code_ids += 1
    ...
    state_vecs = self.cand_model(candidates_input_ids, candidates_input_masks, candidates_segment_ids)[0]  # [bs, length, dim]
...
class BiEncoderShared(nn.Module):
  ...
  def forward(self, context_data=None,candidate_data=None, labels=None,eval=False,get_embedding=False):
    ...
    if get_embedding and context_data is not None:
        context_input_ids,context_segment_ids, context_input_masks =context_data["context_input_ids"], context_data['context_segment_ids'], context_data["context_input_masks"]
        context_vec = self.cont_model(context_input_ids.unsqueeze(0), context_input_masks.unsqueeze(0), context_segment_ids.unsqueeze(0))[-1]  # [bs,dim]
        context_vec = self.context_fc(self.dropout(context_vec))
        context_vec = F.normalize(context_vec, 2, -1)
        return context_vec
    elif get_embedding and candidate_data is not None:
        ...
        candidates_input_masks = candidates_input_masks.view(-1, seq_length)
        candidates_segment_ids = candidates_segment_ids.view(-1, seq_length)
        candidates_vec = self.cand_model(candidates_input_ids, candidates_input_masks, candidates_segment_ids)[-1]  # [bs,dim]
        candidates_vec = candidates_vec.view(batch_size, res_cnt, -1)
        candidates_vec = self.candidate_fc(self.dropout(candidates_vec))
        candidates_vec = F.normalize(candidates_vec, 2, -1)
        return candidates_vec
    ...
    context_vec = self.cont_model(context_input_ids, context_input_masks, context_segment_ids)[-1] 
    batch_size, res_cnt, seq_length = candidates_input_ids.shape
    candidates_input_ids = candidates_input_ids.view(-1, seq_length)
    candidates_input_masks = candidates_input_masks.view(-1, seq_length)
    candidates_segment_ids = candidates_segment_ids.view(-1, seq_length)
