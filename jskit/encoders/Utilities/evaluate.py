...
max_history,max_contexts_length,max_candidate_length,device=None,None,None,None
def config_setup():
...
def get_inference(model,tokenizer,context,candidate):
    global max_history,max_contexts_length,max_candidate_length,device
    context_transform = token_util.SelectionJoinTransform(tokenizer=tokenizer, max_len=max_contexts_length,
                                            max_history=max_history)
    candidate_transform = token_util.SelectionSequentialTransform(tokenizer=tokenizer, max_len=max_candidate_length,
                                                max_history=None, pair_last=False)

    test_data = token_util.SelectionDataset(context,candidate,context_transform, candidate_transform)
    val_dataloader = DataLoader(test_data,
                              batch_size=1, collate_fn=test_data.batchify_join_str, shuffle=False)
