...
RUN cd jsmodels && git lfs pull --include "universal-sentence-encoder_4.tar.gz" && mv models /
RUN cd /models; cd use && tar -xvvzf universal-sentence-encoder_4.tar.gz; cd /;
